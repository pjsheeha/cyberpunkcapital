...
export default function ChapPage(props) {
  console.log(props);
//TODO:
...
//Save Section https://codesandbox.io/s/github/codedailyio/StickyHeaderSections
  ...
  const linkChap = parseInt(props.myUrl) ;
  console.log(linkChap);
  const [chapter, chapterChanger,mountedComponentChapter] = useChapterLoad(linkChap);
  const [section, savedBefore,sectionChanger, savedChanger,mountedComponentSection] = useSectionLoad(linkChap);
      ...
      <div className = "l-content">
        <div>
          ...
          <li className="l-footer__top-nav-cond" id="select-chapter">
            {chapters[elem-1] ?(  <Link className = "save-button"  to={process.env.PUBLIC_URL + "/chap-"+(elem-1) + "#top"} onClick= {()=>{notifSave(0);} } replace>← Previous Chapter </Link>):(<span className="faded-yellow">...</span>)}
            </li>
            <li className="l-footer__top-nav-cond">
            {chapters[elem+1]?(  <Link className = "save-button"  to={process.env.PUBLIC_URL + "/chap-"+(elem+1) + "#top"} onClick= {()=>{notifSave(0);}} replace> Next Chapter ></Link>):(<span className="faded-yellow">More coming soon!</span>)}
            </li>
          </ul>
          <ul className="l-footer__logos" id ="chapters">
          
          {chapters.map((chap,index)=><li> <Link className = "l-footer_button-present"  to={process.env.PUBLIC_URL + "/chap-"+(index) + "#top"} onClick= {()=>{notifSave(0);}} ><h2 className={ elem === index?'faded-yellow':'yellow'}>{Capitalize(chap[0].title)}</h2></Link></li>)}
          </ul>
